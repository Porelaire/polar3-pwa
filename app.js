...
const POLAR3_APP_VERSION = '2.7.13';
...
function pruneDeprecatedUi() {
  const sectionIdsToRemove = [
    ...
    'sec-captura','sec-iluminacion','sec-lightroom','sec-photoshop','sec-montaje','sec-imprenta','sec-archivos','sec-roles','sec-onboarding','sec-glosario','sec-legal-resumen','sec-privacidad','sec-legal-consolidado'
  ];
  ...
  ['flujo', 'economico', 'roles', 'onboarding', 'glosario'].forEach(key => {
  ...
  document.querySelectorAll('button[onclick], a[onclick]').forEach(node => {
    ...
    if (action.includes("showSection('privacidad')") || action.includes("showSection('legal')")) {
      node.setAttribute('onclick', "showSection('forms')");
    }
  });
}
...
function initEvents() {
  ...
  document.querySelectorAll('[data-group-target]').forEach(trigger => {
    trigger.addEventListener('click', evt => {
      evt.preventDefault();
      toggleGroup(trigger.dataset.groupTarget);
    });
  });
